	...
	switch (hash(value, 4, length)) {
		...
		case 6467:
			return (
				...
				value +
				";" +
				(MS + "flex-item-" + value.replace(/flex-|-self/, "") + ";") +
				value
			);
		// align-content
		case 5699:
			return (
				...
				(MS +
					"flex-line-pack" +
					value.replace(/align-content|flex-|-self/, "") +
					";") +
				value
			);
		// flex-shrink
		case 6572:
			return (
				...
				value +
				";" +
				(MS + value.replace("shrink", "negative") + ";") +
				value
			);
		// flex-basis
		case 6316:
			return (
				...
				value +
				";" +
				(MS + value.replace("basis", "preferred-size") + ";") +
				value
			);
		// flex-grow
		case 7084:
			return (
				...
				";" +
				(WEBKIT + value + ";") +
				(MS + value.replace("grow", "positive") + ";") +
				value
			);
		// transition
		case 7626:
			return (
				value.replace(/([^-])(transform)/g, "$1" + WEBKIT + "$2") + ";" + value
			);
		// background
		case 6519:
			return (
				value.replace(/([^-])(image-set\()/, "$1" + WEBKIT + "$2") + ";" + value
			);
		// cursor
		case 7211:
			return (
				...
				switch (charat(value, length + 10)) {
					...
					case 114:
						return (
							...
							value +
							";" +
							(MS + value.replace(/[svh]\w+-[tblr]{2}/, "tb") + ";") +
							value
						);
					// vertical-r(l)
					case 108:
						return (
							...
							value +
							";" +
							(MS + value.replace(/[svh]\w+-[tblr]{2}/, "tb-rl") + ";") +
							value
						);
					// horizontal(-)tb
					case 45:
						return (
							...
							value +
							";" +
							(MS + value.replace(/[svh]\w+-[tblr]{2}/, "lr") + ";") +
							value
						);
				}
			break;
				...
				switch (charat(value, length + 1)) {
					...
					case 109:
						return value.replace(
							/(.+:)(.+)-([^]+)/,
							"$1" + WEBKIT + "$2-$3;" + ("$1" + MOZ + "$2-$3;") + "$1$2-$3"
						);
					// (f)ill-available
					case 102:
						return value.replace(
							/(.+:)(.+)-([^]+)/,
							"$1" + WEBKIT + "$2-$3;" + ("$1" + MOZ + "$3;") + "$1$2-$3"
						);
					// (s)tretch
					case 115:
						return vendor(
							value.replace("stretch", "fill-available"),
							length
		...
		case 7468:
			switch (
				charat(value, strlen(value) - 2 - (value.indexOf("!important") && 10))
			) {
